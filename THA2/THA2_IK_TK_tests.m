...
Tsd = FK_space(S, a, M);
ops = struct('max_iter', 500, 'tol1', 1e-2, 'tol2', .1);
Theta = J_transpose_kinematics(S,Tsd, M, [0;pi/6], ops);
...
g = FK_space(S, a, M);
ops = struct('max_iter', 500, 'tol1', 1e-2, 'tol2', .5);
Theta = J_transpose_kinematics(S,g, M, zeros(size(a)), ops);
